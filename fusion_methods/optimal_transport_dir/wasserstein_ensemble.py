...
from fusion_methods.optimal_transport_dir.ground_metric import GroundMetric
from fusion_methods.optimal_transport_dir.model import get_model_from_name
from fusion_methods.optimal_transport_dir import routines, utils
...
def get_wassersteinized_layers_modularized(args, networks, activations=None, eps=1e-7, test_loader=None):
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in \
        ...
        # print(idx)
        # print("shape: ", fc_layer0_weight.shape)
        ...
        if idx == 0:
            if is_conv:
                # print("beware of RAM issues here: ")
...
def get_network_from_param_list(args, param_list, model):
    new_network = copy.deepcopy(model)
...
def geometric_ensembling_modularized(args, networks, train_loader, test_loader, activations=None):
    ...
    new_network = copy.deepcopy(networks[0])
    return get_network_from_param_list(args, avg_aligned_layers, new_network)


def get_wassersteinized_layers_modularized_not_averaged(args, networks, activations=None, eps=1e-7, test_loader=None):
    '''
    Two neural networks that have to be averaged in geometric manner (i.e. layerwise).
    The 1st network is aligned with respect to the other via wasserstein distance.
    Also this assumes that all the layers are either fully connected or convolutional *(with no bias)*
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in \
        ...
        if idx == 0:
            ...
            if is_conv:
                ...
                print(fc_layer0_weight.data.shape)
                print(T_var.shape)
                if fc_layer0_weight.data.shape[1] != T_var.shape[0]:
                    # Handles the switch from convolutional layers to fc layers
                    fc_layer0_unflattened = fc_layer0_weight.data.view(fc_layer0_weight.shape[0], T_var.shape[0],
        ...
        if is_conv and layer_shape != geometric_fc.shape:
            geometric_fc = geometric_fc.view(layer_shape)
        aligned_layers.append(geometric_fc)
    return aligned_layers


def geometric_ensembling_modularized_multiple_models(args, networks, activations=None):
    assert args.geom_ensemble_type == "wts"
    num_models = len(networks)

    reference_model = copy.deepcopy(networks[0])
    avg_model = copy.deepcopy(networks[0])

    for i, model in enumerate(networks[1:]):
        aligned_param_list = get_wassersteinized_layers_modularized_not_averaged(args, [model, reference_model],
                                                                                 activations)
        layer_idx = 0
        model_state_dict = avg_model.state_dict()
        for key, value in model_state_dict.items():
            model_state_dict[key].data += aligned_param_list[layer_idx].data
            if i == num_models - 2:
                model_state_dict[key].data = model_state_dict[key].data / num_models
            layer_idx += 1
        avg_model.load_state_dict(model_state_dict)
