...
from fusion_methods.optimal_transport_dir.data import get_dataloader
        ...
        if batch_idx % args.log_interval == 0:
            ...
            if args.dump_model:
                ...
                torch.save(optimizer.state_dict(),
                           '{}/{}/optimizer_{}_{}.pth'.format(args.result_dir, args.exp_name, args.model_name,
                                                              model_id))
...
def train_data_separated_models(args, local_train_loaders, local_test_loaders, test_loader, choices):
    ...
    for i in range(args.num_models):
        ...
        network, acc, local_acc = get_trained_data_separated_model(args, i,
                                                                   local_train_loaders[i], local_test_loaders[i],
                                                                   test_loader, base_nets[i])
...
def retrain_models(args, old_networks, train_loader, test_loader, config, tensorboard_obj=None, initial_acc=None,
                   nicks=None):
    accuracies = []
    retrained_networks = []
    # nicks = []
    ...
    for i in range(len(old_networks)):
        ...
        if args.dataset.lower()[0:7] == 'cifar10':
            ...
            retrained_network, acc = cifar_train.get_retrained_model(args, retrain_loader, test_loader, old_networks[i],
                                                                     config, output_root_dir,
                                                                     tensorboard_obj=tensorboard_obj, nick=nick,
                                                                     start_acc=initial_acc[i])
            ...
            start_acc = initial_acc[i]
            retrained_network, acc = get_retrained_model(args, retrain_loader, test_loader, old_network=old_networks[i],
                                                         tensorboard_obj=tensorboard_obj, nick=nick,
                                                         start_acc=start_acc, retrain_seed=args.retrain_seed)
        retrained_networks.append(retrained_network)
        accuracies.append(acc)
    return retrained_networks, accuracies


def intmd_retrain_models(args, old_networks, aligned_wts, train_loader, test_loader, config, tensorboard_obj=None,
                         initial_acc=None):
    accuracies = []
    retrained_networks = []
    # nicks = []
    ...
    for i in range(len(old_networks)):
        ...
        if args.dataset.lower() == 'cifar10':
            ...
            retrained_network, acc = cifar_train.get_retrained_model(args, train_loader, test_loader, old_networks[i],
                                                                     config, output_root_dir,
                                                                     tensorboard_obj=tensorboard_obj, nick=nick,
                                                                     start_acc=start_acc)
            ...
            # start_acc = initial_acc[i]
            retrained_network, acc = get_intmd_retrain_model(args, train_loader, test_loader,
